...
type Board = { id: string; title: string; items: LinkItem[] };
...
const boardsEl = document.querySelector('#boards') as HTMLDivElement;
const quickOpenEl = document.querySelector('#quick-open') as HTMLInputElement;
const quickResultsEl = document.querySelector('#quick-results') as HTMLDivElement;
const newBoardEl = document.querySelector('#new-board') as HTMLButtonElement;
...
function render(): void {
  ...
  state.boards.forEach((board) => {
    ...
    board.items.forEach((item) => {
      ...
      const title = node.querySelector('.card-title') as HTMLAnchorElement;
      title.textContent = item.title;
      title.href = item.url;

      const url = node.querySelector('.card-url') as HTMLParagraphElement;
      url.textContent = item.url;

      const editBtn = node.querySelector('[data-action="edit"]') as HTMLButtonElement;
      editBtn.addEventListener('click', () => editLink(board.id, item.id));

      const deleteBtn = node.querySelector('[data-action="delete"]') as HTMLButtonElement;
    ...
    section.addEventListener('drop', async (e) => {
      ...
      if (!raw) return;
      const { fromBoard, linkId } = JSON.parse(raw) as { fromBoard: string; linkId: string };
      if (fromBoard === board.id) return;
    ...
    const renameBtn = header.querySelector('[data-action="rename-board"]') as HTMLButtonElement;
...
async function addLink(boardId: string): Promise<void> {
...
async function editLink(boardId: string, linkId: string): Promise<void> {
  const board = boardById(boardId);
...
quickOpenEl.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    const first = quickResultsEl.querySelector('a') as HTMLAnchorElement | null;
    first?.click();
  }
});
